...
- **Faint/washed-out partial-refresh text, also confirmed on real hardware** after the framebuffer fix above: the driver never ported the vendor's `EPD_Clear_R26A6H()` step, which syncs the SSD1683's "previous frame" reference RAM (registers 0x26/0xA6) to match what's actually on screen after a refresh. `epd_ll_clear_ram()` wrote `0x00` there once at startup and nothing ever updated it again, so every partial refresh diffed new content against a stale all-black reference — black text pixels looked like "no change" to the partial waveform and barely darkened. Added `epd_ll_sync_reference_ram()` and call it after every refresh `CrowPanel579` triggers (full or partial), so the next refresh always diffs against reality.
