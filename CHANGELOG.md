...
- **Faint/washed-out partial-refresh text, confirmed on real hardware** after the framebuffer fix above: the driver never ported the vendor's `EPD_Clear_R26A6H()` step, which syncs the SSD1683's "previous frame" reference RAM (registers 0x26/0xA6) to match what's actually on screen. `epd_ll_clear_ram()` wrote `0x00` there once at startup and nothing ever updated it again, so every partial refresh diffed new content against a stale all-black reference and barely darkened it. Added `epd_ll_sync_reference_ram()`.
  - First attempt called this after *every* refresh (full or partial), which over-corrected: reflashed and confirmed on hardware, this made previously-drawn content (a border + two lines of text) disappear entirely after a subsequent partial refresh, leaving only the newest content visible — syncing the reference bank to content-bearing full-refresh output made the next partial diff see that content as "unchanged" and it dropped out.
  - Fixed by matching the vendor's proven usage exactly: call `epd_ll_sync_reference_ram()` **once**, in `CrowPanel579::begin()`, with a blank/white framebuffer — never again after a content-bearing refresh. **Reflashed and visually confirmed on real hardware (2026-09-17, v0.1.3): all drawn content (border, both full-refresh text lines, and the partial-refresh text line) now renders correctly and legibly.** Documented known limitation: content *removed* from the framebuffer won't visually clear on a partial refresh until the next full refresh, since the reference bank no longer tracks per-refresh state — not exercised by this template's demo.
