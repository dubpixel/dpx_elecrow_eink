...
**Status:** Firmware template flashed and visually confirmed on real hardware (2026-09-17, v0.1.3) — the demo sketch's border + both full-refresh text lines + the partial-refresh text line all render correctly and legibly. Got there in three hardware iterations: v0.1.1 fixed a framebuffer-geometry bug (black bar + garbled text); v0.1.2 fixed faint partial-refresh text but over-corrected and made previously-drawn content disappear after a subsequent partial refresh; v0.1.3 fixed that by matching vendor's reference-RAM sync pattern exactly (sync once at `begin()`, never after a content-bearing refresh) — see Gotcha #9. This board is a real ESP32-S3 reachable at `/dev/cu.usbserial-10` in this dev environment; flash directly with `pio run -t upload --upload-port /dev/cu.usbserial-10` rather than assuming no hardware is attached. Still unvalidated: WiFi paths, the ghosting-mitigation forced-full-refresh path (never exercised — demo only does 1 partial refresh), deep sleep power draw, and the documented content-removal-doesn't-clear-on-partial limitation.
**Branch:** `main` (PR'd from `feature/crowpanel-579-firmware-template`)
**Version File:** `firmware/VERSION` (currently 0.1.3)
...
9. **`epd_ll_sync_reference_ram()` must be called exactly once, in `begin()`, with a blank framebuffer — not after every refresh.** The SSD1683 pair diffs new content (registers 0x24/0xA4) against a separate reference bank (0x26/0xA6) for partial updates. Two real hardware-confirmed failure modes bracket the correct answer: never sync it past `epd_ll_clear_ram()`'s startup `0x00` and partial refreshes diff against a stale black baseline, coming out faint (v0.1.1→v0.1.2 bug); sync it again after a *content-bearing* refresh (an earlier v0.1.2 attempt did this from both `fullRefresh()` and `partialRefresh()`) and the next partial refresh sees that content as "unchanged" and it visibly disappears, leaving only the newest pixels dark (caught by reflashing and looking at the real panel). The fix that's actually confirmed working on hardware (v0.1.3): sync once, in `begin()`, matching vendor's proven `EPD_Clear_R26A6H()` usage exactly, and never again. This does mean content removed from the framebuffer won't clear via partial refresh until the next full refresh — a documented, accepted limitation, not something to "fix" without a way to verify the change against the real panel again.
